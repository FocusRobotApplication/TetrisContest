...
void Example_6::demo()
{
    ...
    aubo_robot_namespace::ToolInEndDesc toolDesc;  //标定坐标时使用的工具
    ...
    {
        std::cerr<<"运动至准备点.　ret:"<<ret<<std::endl;
    }

    aubo_robot_namespace::Pos position;
    aubo_robot_namespace::ToolInEndDesc toolDesc_1; //运动时的工具
    ...
    for(int i=0;i<10;i++)
    {
        ...
        position.y = 0.0;
        position.z = 0.0;
        //保持当前位姿通过直线运动的方式使工具末端点(toolDesc_1)运动到指定坐标系(userCoord)的指定位置(position)
